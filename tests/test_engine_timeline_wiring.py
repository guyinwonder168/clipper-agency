...
class TestDurationGateWired:
    ...
    def test_over_budget_script_fails_pipeline(
        ...
        with (
            ...
            patch.object(Orchestrator, "_run_scriptwriter", return_value=script_output),
            patch.object(Orchestrator, "_record_gate"),
...
class TestAudioFirstDataFlow:
    ...
    def test_stage_content_returns_2_tuple(
        ...
        settings = _make_settings(target=30, hard=60)
        script_scenes = [
            {"scene": 1, "text": "hook", "word_count": 5, "estimated_duration_sec": 5.0},
        ]
        script_output = {
            "script": script_scenes,
            "caption": "test",
        ...
        pass_gate = GateResult(passed=True, severity="info", message="ok", data={})
        ...
        with (
            ...
            patch(
                ...
                return_value=MagicMock(evaluate=MagicMock(return_value=pass_gate)),
            ),
            patch.object(
                Orchestrator,
    ...
    def test_retry_stops_after_duration_gate_failure(
        ...
        with (
            ...
            patch(
                ...
                if False
                else "clipper_agency.orchestrator.engine.PIPELINE_ORDER",
                [
                    "safety",
                    "segment_producer",
                    "scriptwriter",
            ...
            patch.object(
                Orchestrator, "_fail_agent", return_value={"status": "failed", "job_id": 1}
            ),
            patch.object(
                Orchestrator,
                "_run_content_voice",
