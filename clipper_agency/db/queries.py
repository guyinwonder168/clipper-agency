...
def get_agent_state(
    conn: sqlite3.Connection, job_id: int, agent_name: str
) -> dict[str, Any] | None:
...
def mark_agent_running(
    conn: sqlite3.Connection, job_id: int, agent_name: str, input_data: str | None = None
) -> None:
    """Mark an agent as running and optionally store input_data."""
    update_agent_state(conn, job_id, agent_name, "running", output_data=input_data)


def mark_agent_completed(
    conn: sqlite3.Connection, job_id: int, agent_name: str, output_data: str | None = None
) -> None:
    """Mark an agent as completed and optionally store output_data."""
    update_agent_state(conn, job_id, agent_name, "completed", output_data=output_data)
...
def mark_agent_failed(
    ...
    update_agent_state(
        conn, job_id, agent_name, "failed", output_data=output_data, error_message=error_message
    )
