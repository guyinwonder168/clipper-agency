...
- **Phase 27 / ADR 0030 (Inter-Agent Contract Gates — IMPLEMENTING; FIX-1 COMPLETE PR #84/#85/#86, FIX-6 COMPLETE PR #87, FIX-5 COMPLETE PR #88, FIX-2 COMPLETE PR #91):** job_18 (first fully-completed ElevenLabs job) produced an **unpostable** video. Root cause = a SINGLE load-bearing defect cascading through 5 blind-trust layers: Scriptwriter (`qwen/qwen3-32b`) emitted `narrative_structure` `word_range` covering only words **0–23 of 76** (`_validate_output` checks word-count + emoji, never coverage; `_normalize_narrative_structure` only backfills missing fields) → `build_canonical_timeline` silently stretched the last beat into a **25.17 s mega-beat** → Visual Director planned one wrong-artist scene (Jennifer Coppen for the Sarwendah beat; "accept" = numeric threshold ≥0.60, no entity binding) → Composer `-shortest` cut the last **~2.6 s** ("…like dan share"; visual 32.7 s < audio 35.3 s) → Reviewer total-duration-only `_check_av_sync` PASSED (structurally defeated by `-shortest` equalization) → repair re-derived the timeline from the SAME broken `narrative_structure` → job "completed" garbage (~60 % static: one image ~8 s, near-black "KOMEN DI BAWAH!" card ~24 s). Confirmed via `scripts/diagnose_av_drift.py data/outputs/job_18` + frame contact sheet + AI-vision + a 6-agent research workflow (reference pipelines `MoneyPrinterTurbo-Extended` / `claude-auto-tok` + production short-form tools). **Decision (research-validated): KEEP the 7-agent chain + audio-first beat-driven architecture; ADD deterministic contract gates at every inter-agent boundary + fix the repair router + remove `-shortest`. NOT a restructure** — MoneyPrinterTurbo's opaque-string topology REJECTED (would discard the richer beat-driven contract, which is AHEAD of per-sentence tools and is NOT the job_18 problem). **ADR 0030 amends ADR 0026's no-rebuild default FOR OUTPUT-QUALITY WORK ONLY** (product owner has explicitly lifted the MVP/no-rebuild constraint for output-quality fixes — new gates + new rejection rules are now in scope; still no new agent, no state-machine change, no topology change). Planned gates (SRS FR-74..FR-80, PRD PR-38..PR-44, Design §19/§20): **FIX-1 G7 GateNarrativeCoverage** (assert `word_range` union == `[0, word_count-1]`, contiguous, in-bounds; in-place repair for tail <5 % else force Scriptwriter regen) — THE load-bearing fix; **FIX-6** timeline UNCOVERED_TAIL detection + MAX-beat cap (12 s); **FIX-2 audio-as-master** (drop `-shortest` → `-t voiceover_duration`, pre-render pad visual ≥ audio, G9.5 visual-coverage gate, G10 `AUDIO_NOT_TRUNCATED` re-probe; MoneyPrinterTurbo policy adopted w/o its topology); **FIX-3 entity-binding** at shared `candidate_semantic_ranker` chokepoint (`WRONG_ENTITY` rule — `subject_name` vs beat `spoken_point`+`main_entities`, fuzzy for aliases; VLM `subject_name` required, missing ⇒ revise; `person_match` 0.8→0.6; MAX-scene cap); **FIX-5 repair-router root-cause routing** (route by failure REASON; force narrative regen on coverage re-fail; bounded `MAX_REPAIR_CYCLES` + terminal fail — never "complete" garbage; voiceover-text-diff skip); **FIX-4 Reviewer per-scene** (entity-vs-beat + frozen-frame/max-dwell + audio-not-truncated, extends `_check_av_sync` past the total-duration scalar); **FIX-7 engagement gates** (visual-change-density, hook-on-beat-0, duration-band 21–42 s, monotony guard; WARN+repair not pipeline-death). **DEFERRED Phase 27+:** CLIP image-text relevance ranking; multimodal "watch the rendered video" Reviewer. Ship order: FIX-1 → 6 → 2 → 3 → 5 → 4 → 7, each its own `phase/27-fixN-<slug>` branch + PR + SonarCloud(0 new issues)+Codex(👍)+`--merge`. **FIX-1 COMPLETE (PR #84/#85/#86 — G7 GateNarrativeCoverage + atomic mark-scriptwriter-failed-on-g7). FIX-6 COMPLETE (PR #87 — timeline UNCOVERED_TAIL + MAX-beat cap, backstop for G7). FIX-5 COMPLETE (PR #88 — repair-router root-cause routing + bounded regen + terminal fail + vo-diff skip; 3 Codex P2 state-leaks fixed (coverage-regen terminal re-fail + packaging/promotion failure reset lifecycle status; _promote_repaired_job records manifest final_outputs). FIX-2 COMPLETE (PR #91 — audio-as-master: drop `-shortest` + `tpad=stop_mode=clone` pad + `-t voiceover_duration_sec` cap on BOTH composer builder paths; G10 `AUDIO_NOT_TRUNCATED` audio-STREAM re-probe (`VideoInfo.audio_duration`, immune to container equalization); Reviewer `_check_av_sync` key-mismatch fix (`duration_sec`→`output_duration_sec` — gate was silently INERT, SKIPped on the 0.0); `voice_producer._probe_audio_duration` fail-loud (was silent 0.0 → `voiceover_duration_sec=0.0` on status:success → defeated ALL FIX-2 guards); reviewer zero-duration WARN (not silent SKIP); G10 None→soft_fail. 5-lane ECC review round-2 caught 11 blocking incl. the CRITICAL that Slice A had landed in the LEGACY `_build_assembly_cmd` not the PRODUCTION audio-first path (inert in prod; job_18 ran audio-first) + retry/repair/resume `_run_composer` dropped the param + xfade-overlap pre-compensation (`transition_overlap_sec` in `_compute_audio_master_pad`) + chunk-stitch 0.0 silent-corruption fail-loud. **Slice B (G9.5 pre-render `GateVisualAudioCoverage`) DEFERRED** — redundant w/ FIX-1/FIX-6 (beat_timeline from voice timestamps + `_apply_timeline_to_assets` overrides all asset durations → sum(scenes)≈voiceover enforced upstream) + its xfade-shortfall trigger is now pad-mitigated; revisit only if VD gains a "discover more assets to fill N seconds" mode (Slice B's VD-reroute needs it to deliver value). 2318 offline pass, ruff clean, pyright 0 new, Sonar 0 OPEN, Codex 👍-pending (2 P2 rounds fixed + replied).** **FIX-8 COMPLETE (PR #97, master b1ff863 — word_range cue-anchor contract): job_19 (qwen3-32b) + job_20 (qwen3-30b-a3b-instruct) both terminally failed G7 — empirically proved the LLM cannot reliably count its own words (job_19 over-indexed [52,64] on 60 words; job_20 under-indexed [0,94] on 101). Contract shift (HeyGen pattern): LLM emits `start_cue` (3-5 verbatim first words/beat); CODE derives `word_range` via new `core/beat_anchor.py::derive_word_ranges` (Indonesian tokenizer — strip punct, keep enclitics + internal hyphens for `kata-kata` reduplication; fuzzy `max(Jaccard,LCS)` with ordered-tuple tiebreak; forward-search from `prev_anchor+prev_cue_len` → repeated phrases anchor distinct + overlapping cues rejected). Canonical tokenizer (`beat_anchor.tokenize`/`count_words`) shared by G7 + Scriptwriter so `split()==tokenize()` (voiceover forbids standalone `...`/`—` → indices align 1:1 with Voice Producer timestamps). G7 derivation authoritative when cues present; cue-contract failures route to bounded Scriptwriter regen via `narrative_not_covered` (FIX-5). FIX-5 regen directive rewritten for `start_cue` (was stale `word_range` — root cause of job_19/20 terminal regen fail). Cue-contract fail in `Scriptwriter.run()` stamps `gate_reason` → both initial + repair paths route to regen before `_complete_agent`. 9 codex rounds (all P1/P2 fixed), 5-lane ECC workflow, 2441 offline pass, cov 93%, Sonar 0 OPEN, Codex 👍. **Remaining: FIX-7 (engagement gates) + post-merge validation job_21 (full pipeline downstream FIX-2/3/4/6).** Docs updated: ADR `docs/adr/0030-inter-agent-contract-gates.md`, plan `docs/plans/2026-06-29-inter-agent-contract-gates-tiktok-quality.md` (resume target with per-fix acceptance gates), CHANGELOG/README/SRS 3.4/PRD 3.4/technical_design 5.4/requirements_traceability 4.4. **Pre-PR checklist unchanged:** `.venv/bin/python3 -m pytest -m "not external and not integration" -q` + cov ≥93% + ruff + CHANGELOG + spec docs + Sonar 0-new-issues (verify via `sonarcloud.io/api/issues/search?componentKeys=guyinwonder168_clipper-agency&pullRequest=N&statuses=OPEN,CONFIRMED`) + Codex 👍. Use the workflow-first pattern (Workflow tool: impl + read-only audit + ECC `python-reviewer`/`silent-failure-hunter`) per fix.
