...
### Bug Fixes
- **fix(sonar): tech-debt cleanup C→A — 21 pre-existing SonarCloud issues cleared (Security C→A, Reliability C→A, code smells 18→0):** the project overview showed a C rating on 21 OPEN issues. All 21 were PRE-EXISTING (earliest 2026-05-26, latest 2026-06-30; **0 from the recent FIX-4 PR #94** — the per-PR merge gate enforces 0 NEW issues, not legacy-debt cleanup). The C was driven by Security (2 vuln) + Reliability (1 bug); Maintainability was already A (debt 0.0%, dup 0.0%, cov 94.2%). This PR clears all 21 in two groups. **Group 1 — test-file cosmetics + 1 real bug (19 issues):** (a) `python:S5863` (1, `test_inspection_cache.py:194`) was a REAL useless-test bug — `assert self._key(a) == self._key(a)` is the SAME expression both sides (always True, could never fail); fixed by constructing a distinct identical `twin` candidate so the resume-safe key-stability test actually exercises the comparison (this was the single reliability BUG driving the C). (b) `python:S5778` (14) — `pytest.raises` blocks containing >1 possibly-throwing invocation (`Path(...)`, `str(...)`, `Service()` calls inline); hoisted every non-essential invocation to a local var before the `raises` block so only the single intended throwing call remains inside, across 8 files (`test_rendering_templates.py` ×6, `test_fish_audio.py` ×2, `test_gemini_tts.py`, `test_services_ytdlp.py`, `test_services_elevenlabs.py`, `test_voice_producer.py`, `test_engine_timeline_contract.py`, `test_engine_narrative_coverage_gate.py`). (c) `python:S3415` (4, `test_audio_sequencer.py`) — `assert "[literal]" == actual` swapped to `assert actual == "[literal]"` (actual-left/expected-right). **Group 2 — `pythonsecurity:S6549` path-traversal oracle (2 issues, `core/safe_paths.py:29,55`):** confirmed FALSE POSITIVE — `resolve_existing_file_under`/`probe_video` is called only from `media_probe.py:49` + `engine.py:717`, both passing pipeline-internal job-owned paths (composed video / voiceover output / asset-cache file), NEVER HTTP/end-user input; the `relative_to(base)` containment check (OWASP guard) is present on every resolved path. Sonar flags it because `candidate` flows in through a public function parameter (syntactic taint it cannot model the containment check neutralizing). Restructure: extracted a single `_within_base(base, resolved) -> bool` containment guard (DRYs the 3 inline `try/except relative_to` blocks so the boundary check cannot drift between the relative-primary, CWD-fallback, and absolute branches — `_resolve_relative_candidate` had no direct covering test per codegraph, now exercised through the existing 6 `test_safe_paths.py` regression tests), + `# NOSONAR` with justification on the 2 `.resolve()` sinks citing the established `artifacts.py:19,37` precedent (same oracle, same remedy on internally-constructed paths). Module docstring documents the S6549 FP rationale. **Behavioral note:** the refactor preserves the original `resolved = None` on out-of-base semantics in `_resolve_relative_candidate` (the outer function's `_within_base` re-check would catch an out-of-base return regardless, but matching the original exactly avoids any subtle inner-return change). `# NOSONAR` is the repo-established remedy for confirmed-FP S6549 (the Phase-14 "❌ NOSONAR" lesson was for REAL vulnerabilities being hidden, NOT for false positives on a containment helper — `artifacts.py` is the in-repo precedent that works). **Important:** editing the 2 flagged `safe_paths` lines marks them "new code" — without the NOSONAR suppression they would re-flag as NEW issues and BLOCK the merge gate; the suppression is therefore load-bearing for the gate, not cosmetic. Full offline suite **2410 passed, 22 deselected, cov 93%, ruff 0 new, pyright 0 new** (touched-test-file pyright error counts identical branch-vs-master: the diagnostics are pre-existing baseline noise, e.g. `test_engine_narrative_coverage_gate.py` 32=32). Post-merge expected: Security A, Reliability A, code smells 0.
