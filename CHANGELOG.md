...
### Features
- **AV-drift diagnosis harness (PR 13, ADR-0026 read-only instrumentation):** new `clipper_agency/diagnostics/` package + thin CLI `scripts/diagnose_av_drift.py` that READS persisted artifacts (scriptwriter `narrative_structure.json`, voice_producer `output.json`, muxed `video.mp4`) and probes the muxed video with ffprobe to measure per-beat audio/visual drift. PRIMARY achieved-boundary signal = ffmpeg `blackdetect` (verifier-1 confirmed scene-change `gt(scene,T)` fires 0 cuts on job_8's xfade+Ken-Burns transitions; blackdetect byte-matches the BLACK_FRAME issues already persisted in `visual_coverage.json`, which is reused when present to avoid a subprocess). PLANNED boundaries are a pure cumulative sum of per-beat voiceover spans (word_range INCLUSIVE); predicted-achieved starts mirror `composer.py` L297-328 xfade accumulator. CLI: `diagnose_av_drift.py <job_dir> [--assets-cache] [--pixel-threshold] [--out] [--json]`; exit 0 on success (measurement tool, not a gate), exit 2 on missing inputs. Zero change to any agent/gate/engine. Tests: `tests/diagnostics/` (40 hermetic + 3 `@pytest.mark.integration` ffprobe). Job_8 evidence: `docs/av-drift-diagnosis-job_8.{md,json}`.
