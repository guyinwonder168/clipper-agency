...
### Bug Fixes
- **fix(voice): per-chunk resilience in chunked voiceover (P2, ADR 0029 Phase 2):** `_generate_chunked_voiceover` in `clipper_agency/agents/voice_producer.py` called `service.generate_voice_with_timestamps(chunk, ...)` per chunk with NO try/except, so a single failing chunk raised → caught only by the outer provider-loop → the WHOLE ElevenLabs provider was marked failed → fell through to Gemini. One bad chunk (network blip, provider 5xx) killed the entire primary-TTS voiceover; Phase 0 raised ElevenLabs usage so the trigger probability is now higher. The per-chunk service call is now wrapped in a documented broad `except Exception` (this IS the sanctioned per-chunk isolation boundary — comment explains why): on a chunk failure it logs a warning with chunk index + reason and `continue`s to the next chunk. Only successfully-generated chunks are appended to the three parallel lists (`chunk_paths` / `chunk_timestamps` / `chunk_durations`), keeping them index-aligned for `_concat_audio_chunks` + `_stitch_timestamps` — a skipped chunk is simply absent from all three, so no desync. If EVERY chunk fails (`chunk_paths` empty), the last error is re-raised so the outer ElevenLabs → Gemini → Fish provider-fallback chain still triggers. Signatures unchanged. Tests in `tests/test_voice_producer.py::TestChunkedVoiceoverPerChunkResilience`: (a) chunk 1 succeeds + chunk 2 raises → voiceover still builds from the successful chunks (exception contained); (b) all chunks raise → propagates → Gemini fallback engages. Targeted gate: 31 passed, ruff clean.
