...
### Features
- **FIX-4 Reviewer per-scene detection (ADR 0030, SRS FR-77, PRD PR-41) — closes the job_18 "blind-pass" hole at review time:** the Reviewer's `_check_av_sync` was a total-duration SCALAR (defeated by `-shortest`/`-t` container equalization), and `_evaluate_scene_semantic` only checked (a) matched-beat presence, (b) min scene duration, (c) max beats/scene — it never checked entity binding or audio truncation, so a job_18-shape output (wrong-artist asset in the right time window + audio truncated despite container-duration parity) blind-passed. FIX-4 extends the Reviewer past the scalar with two deterministic per-scene gates, all reusing existing FIX-2/FIX-3 machinery (no new agent / no new gate class / no state-machine change — ADR 0030 compliant). **Slice 1 (subject_name threading):** `RenderedSceneEntry` gains `subject_name: str = ""` (defaults to "" so pre-FIX-4 persisted manifests still deserialize); `build_rendered_scene_manifest` reads it defensively (`scene["subject_name"]` with a nested `inspection["subject_name"]` fallback); composer `_build_scene_manifest` seam copies `assets[i].inspection.subject_name` onto the scene dict via a new pure `_asset_subject_name(asset)` helper (the audio-first PRODUCTION path's `assets` param carries the VD selected-asset dicts with nested `inspection`); `SceneBeatMapping` gains `subject_name` populated in `map_scenes_to_beats` so the reviewer has it without a parallel zip. **Slice 2 (AUDIO_NOT_TRUNCATED defense-in-depth re-probe):** new pure `_check_audio_not_truncated(video_path, voiceover_duration_sec)` RE-PROBES the final video's audio STREAM (immune to container equalization) using the SAME `media_probe.probe_video` + a new shared `media_probe.AUDIO_TRUNC_TOL_SEC = 0.5` constant G10 references — so the two definitions cannot diverge (single source, asserted by an invariant test). Wired as both a programmatic check (`checks["audio_not_truncated"]`) AND a new `_fail_if_audio_truncated` hard gate in the `_fail_if_*` chain. Cannot-verify (probe None / no audio-stream duration) → WARN, never a silent pass (mirrors FIX-2 G10 None→soft_fail). Reason token `AUDIO_TRUNCATED_REVIEWER` (distinct from G10's gate-level `audio_truncated`). **`voiceover_duration_sec` threaded into the reviewer context on BOTH engine `_run_reviewer` call sites** (normal `_retry_review_and_package` + repair `_rerun_upstream_cascade`) — the FIX-2 PR #91 blast-radius lesson: a happy-path-only wire is inert in retry/repair. **Slice 3 (per-scene ENTITY-VS-BIND):** new pure `_run_entity_binding_review(mappings, story_beats, main_entities)` REUSES FIX-3's `derive_expected_entities` + `entity_overlap` (DRY — single entity-binding contract). Per scene mapped to a person/entity beat: subject_name present + wrong entity → reject `ENTITY_MISMATCH`; subject_name empty on an entity beat → WARN (`ENTITY_UNVERIFIABLE`, recorded, NOT a hard-fail so a pre-FIX-4 persisted manifest doesn't death-loop — cannot verify ≠ verified good, mirror of FIX-3 `is_unverifiable_entity_binding`); non-person beat → skip (backward compat). Wired as `_fail_if_entity_mismatch` in the chain; entity reviews surfaced in output `entity_binding_reviews`. **Slices 4-5 (entity_overlap robustness):** extended `_GENERIC_CONTRACT_WORDS` with platform/format words (tiktok, youtube, ig, instagram, reels, duet, stitch, fyp, konten, kreator) so a "TikTok" asset never matches a beat that merely mentions TikTok; `entity_overlap` filters generic words from the expected set (defense-in-depth) AND adds a phrase-level exact-match before the per-token matcher (handles full-name phrases like "Jennifer Coppen" cleanly, no regression on the existing single-token char-pinning tests). **Slice 6 (repair routing):** `ENTITY_MISMATCH` → Visual Director (replace_visual); `AUDIO_TRUNCATED_REVIEWER` → Composer (redo_compose); new `wrong_entity` reason in `route_repair._by_reason`. MAX_REPAIR_CYCLES bounding (FIX-5) intact — repeated ENTITY_MISMATCH terminal-fails, never "completes" garbage. **DEFERRED (flagged, owner-confirmed out of slices 56-62):** plan §FIX-4 item 3 MAX-DWELL/frozen-frame (>4s static card → FIX-7 engagement territory) and item 4 uncovered-tail-scene (redundant with FIX-6 UNCOVERED_TAIL already shipped). **Round-3 codex hardening (post-workflow operator adjudication — the 5-lane + codex loop surfaced these, all fixed + regression-tested):** (a) the per-scene WRONG_ENTITY gate was structurally INERT on the production audio-first path — `_candidate_to_action` stripped the matched candidate to `{type, source_url}` so the executed asset never carried `subject_name` and `_asset_subject_name` always returned `""`; fixed by stashing `subject_name`+`asset_id` from the matched candidate's inspection in `_apply_best_candidate` + threading them through `_execute_beat_plan` onto the executed asset (the FIX-2 "fix landed in the legacy builder" lesson — caught by the codex + blast-radius lanes), pinned by a full VD.execute→Composer→rendered_scene_manifest regression test driving the real seam. (b) `_entity_expected_for_beat` no longer appends global `main_entities` to a beat with no beat-local entity — would false-positive ENTITY_MISMATCH a legit platform/format asset on a non-person beat now that subject_name flows. (c) fallback/text-card (`source:none`) assets no longer carry the failed primary candidate's `subject_name`/`asset_id` (guarded on `primary_rendered = primary_result is not None`) — else the gate would falsely PASS an unverified fallback visual; regression test asserts the fallback asset's subject is `""`. (d) `engine.py` reviewer-call comment corrected — package-consistency gate additionally needs `story_mode_decision`/`thumbnail_text` (pre-existing gate inertness, NOT FIX-4 scope, deferred). **Codex P1+P2 (round-4):** (e) `_URL_FAITHFUL_SOURCES = {tiktok_clip}` guard — only candidates rendered via a URL-faithful executor (yt-dlp downloads the inspected URL verbatim) carry `subject_name`; `pexels_image`/`pexels_video` SEARCH Pexels by query (a different asset than inspected), so they now degrade to `subject_name=""` → ENTITY_UNVERIFIABLE instead of falsely verifying an uninspected visual. (f) full phrase-level `entity_overlap` refactor (the workflow's "phrase exact-match" was a half-measure that left flat per-token matching): `_entities_from_text`/`derive_expected_entities` now return `list[list[str]]` phrases (maximal runs of consecutive proper-noun tokens; stopwords/generics break a run); single-token phrase stays lenient (Rules A/B/C — keeps Cristiano-Ronaldo/ronaldo + alias tolerance), multi-token phrase requires token-set Jaccard ≥ 0.75 so a shared token ("ahmad" between "Raffi Ahmad" and "Ahmad Doe") can't accept a different person. Pinned by `test_multi_token_shared_token_no_match`. Tests: 7 manifest/subject_name-threading + 6 reviewer AUDIO_NOT_TRUNCATED (pass/fail/warn/skip/execute-hard-fail/single-source-tolerance) + 8 entity-binding (mismatch hard-fail w/o LLM, correct pass, empty-subject WARN, non-person skip, pure-helper unit) + 11 entity_overlap/platform-word (parametrized) + 4 repair-routing + 1 engine blast-radius (voiceover_duration_sec reaches reviewer on the normal path); 2 pre-existing reviewer tests updated for the new `audio_not_truncated` programmatic key. Full offline suite green, ruff 0 new on FIX-4 lines. Traceability: SRS FR-77, PRD PR-41.
...
- **FIX-5 repair-router root-cause routing + bounded regen + terminal fail (ADR 0030, SRS FR-78, PRD PR-43) — closes the job_18 repair-loop hole:** the repair router routes by failure REASON to the ROOT agent instead of re-deriving the timeline from the same broken structure every cycle (the job_18 recurrence mechanic). `core/repair_router.py` maps the two stable coverage tokens emitted by FIX-1/FIX-6 — `narrative_not_covered` (G7) + `timeline_not_covered` (FIX-6), both in `GateResult.data["reason"]` — to a Scriptwriter `narrative_structure` REGENERATION patch with an explicit "cover ALL words" instruction (it does NOT rebuild `beat_timeline` from the known-broken structure). Bounded `_COVERAGE_MAX_REPAIR_CYCLES = 1`: after the regen attempt, a coverage re-fail TERMINALLY FAILs the job (never silently "completes" a garbage video — the anti-job_18 invariant, pinned end-to-end with `_execute_repair_cycle` NOT mocked). Coverage-abort routing wired at all 3 abort sites: `_stage_content` (G7 narrative), `_stage_composition` (FIX-6 timeline), + the reviewer-stage 2nd-hop in `_retry_review_and_package`. Voiceover-text-diff skip (claude-auto-tok pattern): a Scriptwriter revision that leaves `voiceover_text` unchanged skips the expensive Voice Producer call + reuses cached audio/timestamps; regenerates only when the text actually changed. Resilience: `_load_previous_patches` catches `(json.JSONDecodeError, OSError)` so a corrupt internal bookkeeping file can't mask the root cause + pre-empt the single regen attempt; `_promote_to_final` drops `cycle=repair_cycle` so a repaired job promotes the PACKAGED `job_{id}/` dir (with `metadata.json`) instead of the interim `cycle_{n}` snapshot that lacked it (broken-output-package-contract bug). `db/queries.py`: a COMPLETED transition clears the stale failure `error_message` (NULL on COMPLETED, COALESCE-preserve on FAILED). ECC workflow (design → RED → GREEN → 5-lane review) + manual finish of 12 test `None`-deref pyright errors (test-only `Optional` subscripts — `assert ... is not None` / walrus-narrow). Traceability: SRS FR-78, PRD PR-43. Full offline suite green (2294 pass), ruff clean. The 83 pre-existing pyright errors (engine.py `GateResult`/`dict` annotation, `queries.py` `int|None`, etc.) are NOT on FIX-5-added lines (git-diff-master-proven) → out of scope, separate tech-debt PR.
- **FIX-3 round-2/3 hardening (ADR 0030, ECC workflow codex review — closed 3 issues the bot's single pass missed + the long-deferred cache-key §8 follow-up):** (1) **cache-key drift regression** — FIX-3 Slice 1 widened the VLM prompt with `visual_must_show`/`visual_must_not_show` but the inspection cache key (`beat_claim=spoken_point`, `prompt_version="1.0"`) didn't change, so a pre-widen cache entry was a stale hit. Fixed properly by extracting a shared `inspection_cache.compute_candidate_cache_key(candidate, spoken_point, visual_must_show, visual_must_not_show)` — the ADR 0027 §8 follow-up the `asset_qualification.py` header explicitly deferred — now the single source of truth called by BOTH VD `_score_one_candidate` and AQ `_score_candidate`, with an evidence-contract hash covering `visual_must_show`/`visual_must_not_show` (removes drift by construction). (2) **multi-person narration bypass (Codex P1)** — `derive_expected_entities` merged `visual_must_show` + `spoken_point`, so a Sarwendah beat whose narration mentioned "Jennifer Coppen" for contrast added `jennifer`/`coppen` to the binding set → a Jennifer asset PASSED `WRONG_ENTITY` (the exact job_18 bug). Fixed: `visual_must_show` is AUTHORITATIVE — when it yields named entities those are returned ALONE; `spoken_point` is a fallback ONLY when `visual_must_show` yields none. (3) AQ excludes unverifiable-entity candidates from the qualified set; added VD production-path stale-cache guard test + AQ recovery regression. **Deferred to CLIP / Phase 27+ (Codex P2, non-blocking):** `entity_overlap` single-token over-match for multi-token names (e.g. "Ahmad Doe" shares `ahmad` with expected "Raffi Ahmad") — name-matching precision needs the CLIP image-text ranker ADR 0030 planned; the job_18 class (completely-different names like Jennifer Coppen vs Sarwendah) is correctly rejected today. ECC review-only workflow (ran full despite `review_only`/`skip_codex` flags): blast-radius + silent-failure APPROVE; codex 3 rounds → 2 P2 + 1 P1 fixed, 1 non-blocking P2 deferred. Full offline suite **2364 passed**, 22 deselected, ruff 0 new on FIX-3 lines.
- **FIX-3 entity-binding rejection at the qualification/VD chokepoint (ADR 0030, SRS FR-76, PRD PR-40) — kills the job_18 "wrong artist" class:** "accept" was a numeric threshold with NO entity binding, so a beat about **Sarwendah** accepted a **Jennifer Coppen** image (high `person_match` — it IS a person — + moderate `claim_support` → `misleading_risk=0.0` → accepted). The VLM was never asked to NAME the subject, and nothing compared a depicted subject's name to the beat's target entity. FIX-3 adds a deterministic `WRONG_ENTITY` rejection rule at the shared `candidate_semantic_ranker.apply_rejection_rules` chokepoint (the single source of truth called by BOTH `rank_candidates` AND AQ `_rank_and_select`'s standalone reject-reasons pass — so it fires on both the pre-VD qualification boundary and VD's own accept step). **Slice 1 (VLM emits `subject_name`):** new `subject_name` string field end-to-end — `_INSPECTION_FIELDS` prompt + example JSON, `parse_inspection_json` coerces it (`str(... or "")`, OUTSIDE `_SCORE_KEYS` since it's a string not a bounded score), `_build_result` + `_error_result` carry it, `AssetSemanticInspection` schema gains `subject_name: str = ""`. Also widened the VD inspector beat dict (`visual_director.py` `_run_multimodal_inspection`) to pass `visual_must_show` + `visual_must_not_show` (previously only `spoken_point` reached the VLM, so it had no evidence contract when naming the subject). **Slice 2 (WRONG_ENTITY rule + pure helpers):** new `derive_expected_entities(spoken_point, visual_must_show)` (tokenizes beat text — curated `visual_must_show` alpha tokens ≥4 + capitalized proper-noun tokens from free-text `spoken_point`, Indonesian+English stopwords dropped; tuned for RECALL: extra junk tokens are harmless, a MISSED true entity is the costly false-reject) + `entity_overlap(subject_name, expected)` (3 rules: exact token · bidirectional substring ≥4 for aliases/transliteration like Sarwenda≈Sarwendah · char-set Jaccard ≥0.75 for ≥5-char spelling drift). Verified: Jennifer Coppen vs sarwendah scores Jaccard 0.25 → no match → WRONG_ENTITY. `apply_rejection_rules` gains WRONG_ENTITY as its FIRST (most-specific) rule. The `expected_entities` list is decorated onto BOTH scored-dict builders (`asset_qualification._score_candidate` + `visual_director._score_one_candidate`), each deriving it from its own `beat.spoken_point`/`beat.visual_must_show` — the engine seam forwards ORIGINAL `AssetCandidate` objects (not AQ's scored dicts) to VD, so each site rebuilds the decoration independently (can never vanish). **Backward-compat gating:** WRONG_ENTITY + the Slice-3 downgrade fire ONLY when `expected_entities` is non-empty (non-person / undecorated beats + all existing fixtures are byte-unaffected — the rule is a no-op). **Slice 3 (missing `subject_name` ⇒ revise):** in `rank_candidates`, an `accept` whose inspection depicts a person (`person_match ≥ 0.5`) but whose subject the VLM could not name (empty `subject_name`) on an entity-binding beat is downgraded to `revise` — a person-asset that can't be entity-verified is never the confident pick. **Slice 4 (threshold 0.8→0.6) DEFERRED to FIX-3.5:** Slices 1+2+3 close job_18 deterministically without it; `_compute_misleading_risk` had ZERO covering tests, the change needs a `min(1.0,…)` clamp smell, would double-penalize with WRONG_ENTITY (muddying `reject_reasons` telemetry), and risks fleet-wide false-positives on legitimate headshots. New characterization-pinning tests LOCK the current 0.8 behavior so the future change is deliberate. **R-1 stale-cache guard (HIGH):** a pre-FIX-3 cached inspection lacks the `subject_name` KEY → on resume/retry the Slice-3 downgrade would mass-downgrade every stale person-asset to "revise"; BOTH scored-dict builders now treat `cached.get("subject_name") is None` as a cache MISS (re-inspect, self-healing — one re-inspect per stale entry, then the cache is correct forever). Distinguishes KEY-ABSENT (stale → re-inspect) from `subject_name=""` (present → use cache). **Cache-key parity PRESERVED:** `subject_name` lives in the inspection VALUE + `expected_entities` on the scored dict — NEITHER enters `compute_cache_key` (AQ constants `_CACHE_MODEL`/`_CACHE_PROMPT_VERSION`/`_CACHE_EVIDENCE_CONTRACT_HASH` byte-identical to VD literals, untouched); ADR 0027 AQ-delegates-to-VD-bound-inspection ⇒ byte-identical cached output ⇒ `subject_name` lands identically. FIX-6's `MAX_BEAT_DURATION_SEC=12` already subsumes the plan's "max-scene cap" half — not redone. Tests: 24 new entity-binding tests (WRONG_ENTITY rejection, alias tolerance, Jennifer≠Sarwendah safety, missing-subject downgrade, all-wrong fallback) + `_compute_misleading_risk` char-pin + `subject_name` end-to-end (parse/build/error) + R-1 stale-cache reinspection regression (stale→re-inspect, fresh→cache-hit) + cache-key parity; the 8 pre-existing cached-inspection fixtures updated to carry `subject_name` (the field is now part of the inspection contract). ECC workflow attempt (run wf_45f9e2b3-f79) FAILED — its Impl lane was blocked by plan mode (zero code landed) + its Design agent ignored the Slice-4 DEFER constraint; reimplemented directly per the validated plan + `review_only` review pending. Full offline suite: **2353 passed, 22 deselected, cov 93%, ruff 0 new on FIX-3 lines.** Traceability: SRS FR-76, PRD PR-40.
...
### Bug Fixes
- **fix(sonar): exclude the 2 confirmed-FP S6549 → Security C→A (project-level `sonar.issue.ignore.multicriteria`):** PR #95 cleared 19/21 Sonar issues (Reliability A, code smells 0) but the 2 `pythonsecurity:S6549` path-traversal findings in `core/safe_paths.py` remained — they sit in master's new-code leak period so they kept the Security rating at C. `# NOSONAR` cannot suppress them because S6549 is a TAINT-analysis rule (NOSONAR suppresses only static rules — empirically confirmed on PR #95, where it also raised `python:S7632`). The documented remedy for a confirmed taint-rule false positive is a PROJECT-LEVEL issue exclusion (not line-level): `sonar-project.properties` now carries `sonar.issue.ignore.multicriteria=s6549_safe_paths` filtering `pythonsecurity:S6549` for `**/safe_paths.py`. Verified empirically — PR #96 Sonar analysis shows **0** S6549 (the exclusion is honored at issue-generation, unlike NOSONAR). Inline justification documents the FP rationale: `safe_paths.py` IS the OWASP containment boundary (every caller — `media_probe.probe_video`, `engine` voiceover-regen — passes a pipeline-internal job-owned path; Flask surface ingests only topic/niche/job_id, none reach these functions; `Path.relative_to(base)` containment enforced on every branch + `.resolve()` follows symlinks). Self-disarming note: if a future caller passes genuinely user-controlled input to `resolve_existing_file_under`, REMOVE the exclusion so S6549 re-arms. Config-only change (no Python code, no test/ruff/pyright impact). With PR #95 + #96 together: **21 → 0 OPEN, all ratings A** (Reliability A, Security A, Maintainability A, smells 0).
- **fix(sonar): tech-debt cleanup — Reliability C→A + code smells 18→0 (19 of 21 cleared; 2 Security S6549 are confirmed-FP pending Sonar UI mark):** the project overview showed a C rating on 21 OPEN issues. All 21 were PRE-EXISTING (earliest 2026-05-26, latest 2026-06-30; **0 from the recent FIX-4 PR #94** — the per-PR merge gate enforces 0 NEW issues, not legacy-debt cleanup). The C was driven by Security (2 vuln) + Reliability (1 bug); Maintainability was already A (debt 0.0%, dup 0.0%, cov 94.2%). This PR clears 19 (Reliability bug + all 18 code smells); the 2 Security S6549 are confirmed false positives but cannot be suppressed in-code (see DEFERRED). **Group 1 — test-file cosmetics + 1 real bug (19 issues):** (a) `python:S5863` (1, `test_inspection_cache.py:194`) was a REAL useless-test bug — `assert self._key(a) == self._key(a)` is the SAME expression both sides (always True, could never fail); fixed by constructing a distinct identical `twin` candidate so the resume-safe key-stability test actually exercises the comparison (this was the single reliability BUG driving the C). (b) `python:S5778` (14) — `pytest.raises` blocks containing >1 possibly-throwing invocation (`Path(...)`, `str(...)`, `Service()` calls inline); hoisted every non-essential invocation to a local var before the `raises` block so only the single intended throwing call remains inside, across 8 files (`test_rendering_templates.py` ×6, `test_fish_audio.py` ×2, `test_gemini_tts.py`, `test_services_ytdlp.py`, `test_services_elevenlabs.py`, `test_voice_producer.py`, `test_engine_timeline_contract.py`, `test_engine_narrative_coverage_gate.py`). (c) `python:S3415` (4, `test_audio_sequencer.py`) — `assert "[literal]" == actual` swapped to `assert actual == "[literal]"` (actual-left/expected-right). **DEFERRED — `pythonsecurity:S6549` path-traversal oracle (2 issues, `core/safe_paths.py:29,55`) — confirmed FALSE POSITIVE, cannot be code-cleared:** the 2 S6549 are confirmed FP (5-lane security review traced every caller of `resolve_existing_file_under`/`probe_video` — `media_probe.py:49` + `engine.py:717`, both pass pipeline-internal job-owned paths: composed video / voiceover output / asset-cache file, NEVER HTTP/user input; `relative_to(base)` containment is enforced on every branch; the Flask surface ingests only topic/niche/job_id, none of which reach these functions). HOWEVER the 2 issues cannot be suppressed in-code: (1) `# NOSONAR` does NOT suppress Sonar **taint-analysis** rules like S6549 (it suppresses only static rules) — empirically confirmed across 2 PR analyses: placing `# NOSONAR` on the `.resolve()` sink line left S6549 flagged AND added 2 `python:S7632` (non-compliant-NOSONAR) issues, net worse; (2) no `SONAR_TOKEN` is available outside GitHub Actions secrets (`secrets.SONAR_TOKEN` in `.github/workflows/build.yaml`), so the issues cannot be transitioned to `falsepositive`/`wontfix` via API from a local session. The ONLY remedy is marking the 2 issues "False Positive" in the SonarCloud UI (a 1-minute manual action for an account with project-admin rights), which removes them from the OPEN count → Security C→A. A `_within_base` containment-guard extraction (DRY refactor of the 3 inline `try/except relative_to` checks) was drafted + passed the 5-lane review but was REVERTED from this PR because ANY edit to the flagged `safe_paths.py` lines marks them new-code → 2 NEW S6549 → merge-gate block (with no in-code suppressor available); the refactor can ship as a standalone PR AFTER the UI FP-mark clears the baseline. Note: `artifacts.py:19,37` `# NOSONAR` "works" only because its taint chain does not trace to a flagged sink — it is NOT a valid precedent for clearing S6549. Full offline suite **2410 passed, 22 deselected, cov 93%, ruff 0 new, pyright 0 new** (touched-test-file pyright error counts identical branch-vs-master: the diagnostics are pre-existing baseline noise, e.g. `test_engine_narrative_coverage_gate.py` 32=32). Post-merge expected: **Reliability A, code smells 0; Security stays C** (2 confirmed-FP S6549 pending Sonar UI mark — see DEFERRED).
- **fix(composer): FIX-2 audio-as-master (ADR 0030, SRS FR-79.5, PRD PR-41) — kills the job_18 "last 2.6s audio cut" class:** Composer's `-shortest` flag cut the output to the shortest stream (visual 32.7s < audio 35.3s → audio truncated, the "…like dan share" CTA lost), and the downstream gates were structurally unable to detect it. Audio is now the MASTER. Three slices + a 5-lane review round-2 that caught a critical production-path miss. **Slice A (composer):** drop `-shortest` from BOTH builder paths; pre-render pad the visual to fill the voiceover via new DRY helper `_compute_audio_master_pad` (extends the last PATH-BEARING scene's trim by the gap when `sum(target_duration) < voiceover_duration_sec - 0.3`); append `tpad=stop_mode=clone` after trim (trim alone does NOT hold the last frame for a source shorter than the padded duration — reliable only for zoompan images; tpad deterministically clones); cap output with `-t voiceover_duration_sec` (fallback `-t sum+1s` when absent, preserving the safety bound `-shortest` provided). **Slice C (media_probe + gates + engine):** `VideoInfo` gains `audio_duration` (per-audio-stream, distinct from the `-shortest`/`-t`-equalized container `format.duration`); `GateVideoValidation` (G10) takes `voiceover_duration_sec` + hard_fails `AUDIO_NOT_TRUNCATED` when `audio_duration < voiceover_duration_sec - 0.5s` (soft_fail when the caller opted-in but `audio_duration` is None — recorded, non-aborting; never silent-pass); wired at BOTH G10 sites (normal `_stage_composition` + `_retry_composer_stage`). **Slice D (engine + reviewer):** the reviewer AV-sync hard gate was silently INERT — engine passed `compose_output['duration_sec']` (a NON-EXISTENT key; composer writes `output_duration_sec`) as `visual_duration_sec` → 0.0 → `_check_av_sync` SKIPped. Fixed both reviewer call sites (L617 + L2688). `_check_av_sync` now WARNs (visible) when exactly one duration is 0 instead of silently SKIPping. **5-lane review round-2 (the critical catches):** (1) codex P1 + blast P1 — Slice A originally landed ONLY in the legacy `_build_assembly_cmd`, but production runs the AUDIO-FIRST path (`_build_audio_first_cmd`) which still had `-shortest` → Slice A was INERT in production (job_18 ran audio-first); threaded `voiceover_duration_sec` through the production audio-first chain + engine `_retry_composer_stage` L2592 (retry + both repair cascades + resume); (2) silent P1 — `voice_producer._probe_audio_duration` silently returned 0.0 on any probe failure → stamped `voiceover_duration_sec=0.0` on `status:success` → defeated ALL FIX-2 guards (the same job_18 silent-bypass class, now load-bearing); now RAISES on failure (tolerant callers `_approximate_timestamps` + chunk stitching catch). Slice B (G9.5 pre-render `GateVisualAudioCoverage`) DEFERRED — redundant with FIX-1 (G7 word_range) + FIX-6 (UNCOVERED_TAIL): beat_timeline is built from voice timestamps + `_apply_timeline_to_assets` overrides every asset (incl card fallbacks) with timeline `target_duration` → `sum(scene durations) == sum(beat durations) ≈ voiceover`, already enforced upstream. ECC workflow (manual impl → 5-lane review: `python-reviewer` + `silent-failure-hunter` + `pr-test-analyzer` + blast-radius + local `codex review` → loop round 2 → codex APPROVE). **Non-blocking follow-up (codex r2 P2, deferred to Slice B):** xfade transition overlap is not pre-compensated in the audio-first pad (a ready helper `_inflate_durations_for_transitions` exists, no production callers) → bounded ~0.5s×(N-1) frozen-frame tail on the visual stream; strictly better than the `-shortest` audio truncation; G9.5 is the planned closure. Full offline suite: 2317 passed, 18 deselected, ruff clean, pyright 0 new (11 pre-existing on master).
- **fix(orchestrator): pyright tech-debt — 15 pre-existing type errors in the orchestrator core (the deferred tech-debt PR FIX-5 referenced):** the 15 pre-existing pyright errors in `engine.py` + `db/queries.py` that generated the diff-context noise which broke FIX-5's review loop are now 0. This is the "separate tech-debt PR" FIX-5 explicitly deferred (`engine.py GateResult/dict annotation, queries.py int|None, etc. → out of scope, separate tech-debt PR`). Behavior-preserving on the happy path; every fix is either type-only or an explicit fail-fast improvement on a previously-silent-impossible-state path. `db/queries.py`: `create_job` (L43) + `create_agent_state` (L114) now `raise RuntimeError` when `cursor.lastrowid is None` instead of returning `None` as an `int` (unreachable after INSERT into an INTEGER PRIMARY KEY table — documents the invariant, pyright-clean, fails loud instead of corrupting). `engine.py`: (1) `_stage_safety` return annotation `tuple[int, dict[str,Any]]` → `tuple[int, GateResult]` — type-only, zero call-site changes (verified via codegraph: sole caller `run_pipeline` reads `cost_result.data.get(...)` which is a `GateResult` attribute, not a dict method); (2) the 5 `script_output`/`voice_output` `dict|str` call-site errors (L2157/2158/2178/2184/2209) fixed by an `isinstance(stage3, tuple)` guard before unpacking — the `str` variant came from unpacking a failure dict, all of which are caught by the preceding `isinstance` check at L2139 (unreachable on the happy path); (3) the `review_output.get` None-deref (L2231/2232) + `cycle_dir` possibly-unbound (L2580/2581/2584) fixed with non-strippable `raise RuntimeError(...)` + `cycle_dir: Path | None = None` init — NOT `assert` (ECC `silent-failure-hunter` P2: `assert` is stripped under `python -O`, leaving a bare `None.get` AttributeError on the production COMPLETION path that the broad `except` would rewrite as FAILED — hiding the contract violation; replaced with `raise` which survives `-O`, consistent with the existing L2150 contract-guard pattern). Two hermetic regression tests (`tests/test_db_queries.py`) lock the `lastrowid=None` raises via a `MagicMock` connection. ECC workflow (impl → 4-lane review: `python-reviewer` + `silent-failure-hunter` + blast-radius + local `codex review` → loop round 2 → codex clean). Full offline suite: 2296 passed, 18 deselected, ruff clean, pyright 0 on `engine.py` + `queries.py`. **Deferred to separate tier PRs:** `services/*` (46 more package errors) + `tests/*` (197 errors — mostly `Optional` subscripts in pre-existing test code; only noise up fix PRs that add tests). Unblocks trusting overnight `ecc-review-loop` auto-runs on the orchestrator path (the noise that caused FIX-5's 2h non-convergent loop is gone).
