...
`config/preflight.py` → `preflight_agent_models()` runs at startup (in `__main__.py run()`, before `run_pipeline`, and on `--dry-run`): it force-refreshes the cache, resolves every LLM-backed agent's model, and validates each slug is a key in the live catalog. A miss against a **populated** catalog fails fast (clear error, exit 1) before billing research credits; when no catalog is reachable (offline + no cache) it logs a warning and continues rather than block every run. The dead `llm/router.py` (`ModelPreset`/`resolve_model`, 0 production consumers) was removed in the same PR.
