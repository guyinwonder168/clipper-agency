...
|---|---|---|
| **G7 GateNarrativeCoverage** — `word_range` union == `[0, word_count-1]`, contiguous, in-bounds; in-place repair for tail <5 % else force Scriptwriter regen. **FIX-8 (cue-anchor):** the LLM cannot reliably count words (job_19 over-index, job_20 under-index), so the Scriptwriter emits `start_cue` (3-5 verbatim first words of each beat); the engine derives `word_range` via `core/beat_anchor.py::derive_word_ranges` (fuzzy `max(Jaccard, LCS) ≥ 0.6`, deterministic Indonesian tokenizer). Cue failures (`cue_not_found` / `cue_out_of_order`) hard-fail with the stable `narrative_not_covered` token → FIX-5 routes to Scriptwriter. FIX-1 word_range validator kept as defense-in-depth + legacy fallback | `orchestrator/gates.py`, `engine._stage_content` + `_derive_or_validate_coverage`, `scriptwriter._validate_output` + `_normalize_narrative_structure`, `core/beat_anchor.py` | FR-74 / PR-38 |
...
| **Reviewer per-scene entity-vs-beat + frozen-frame/max-dwell + audio-not-truncated** (FIX-4: entity-vs-beat + audio-not-truncated COMPLETE via `_run_entity_binding_review` reusing FIX-3 `derive_expected_entities`/`entity_overlap`, `_check_audio_not_truncated` reusing `media_probe.AUDIO_TRUNC_TOL_SEC`; `subject_name` threaded VD→`rendered_scene_manifest`→`SceneBeatMapping`; `voiceover_duration_sec` wired on both engine reviewer paths; reason tokens `ENTITY_MISMATCH`/`AUDIO_TRUNCATED_REVIEWER` routed via `repair_router`. MAX-DWELL/frozen-frame DEFERRED to FIX-7) | `agents/reviewer.py`, `core/reviewer_context.py`, `core/rendered_scene_manifest.py`, `agents/composer.py`, `core/candidate_semantic_ranker.py` | FR-77 / PR-41 |
