...
| `_run_inspection` | `(candidate: Any, beat: StoryBeat, job_id: int, cache_dir: str, cache_key: str, agent_dir: str, inspector: Callable[..., dict \| None] \| None) -> dict \| None` | Originally specced as a verbatim lift of `VD._run_multimodal_inspection` (visual_director.py:895). **[ADR 0027 / SLICE 6 — superseded]** Implemented as DELEGATION: forwards the cache-miss call to the injected `inspector` (the engine seam binds `VisualDirectorAgent._run_multimodal_inspection`), returning `None` when no inspector is configured. Reuses VD's exact inspection — frame ownership stays in VD (no double extraction **[V2/V4]**) and the cached output is byte-identical to VD's (no cache-namespace drift / no double-VLM) — without lifting ~140 lines of OCR/face/enhanced ML machinery. A verbatim lift was rejected (ADR 0026 "do not rebuild" tension + drift risk); a minimal lift was rejected (diverges from VD under the same cache key). |
