...
| FR-74 | **Narrative-coverage contract gate (G7 GateNarrativeCoverage, ADR 0030 — job_18 root cause):** a deterministic gate asserts that Scriptwriter `narrative_structure` `word_range` indices fully cover `voiceover_text` — (a) every `word_range` in-bounds `[0, len(words)-1]`; (b) beats sorted + contiguous (`beat[i].end+1 == beat[i+1].start`); (c) the union of `word_range` == `[0, len(words)-1]` within tolerance (last beat `end >= len-1 - floor(len*0.05)`). Enforced AFTER Scriptwriter, BEFORE Voice Producer (in `engine._stage_content`), and mirrored as `_validate_narrative_coverage` inside `scriptwriter._validate_output`. On failure: in-place repair (extend last beat / insert a closing beat covering the tail words) ONLY when the uncovered tail is <5 % of words; otherwise hard-fail routing repair to **Scriptwriter** (NOT Visual Director) with an explicit "regenerate beats covering ALL words" instruction. job_18 root cause was a `word_range` covering only words 0–23 of 76 — 52 words / ~23 s of narration collapsed into one 25-second closing scene. ADR 0030 amends ADR 0026's no-rebuild default FOR OUTPUT-QUALITY WORK ONLY (product owner has lifted the constraint). Pure contract enforcement — no new agent, no state-machine change (a new gate, which ADR 0030 authorizes) | P0 | ADR 0030 |
