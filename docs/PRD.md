...
| PR-30 | Deterministic visual quality gates, package consistency, semantic visual relevance, and structured repair routing: Reviewer runs deterministic gate chain (visual coverage → text collision → safe area → package consistency → timestamp semantic → semantic relevance) before LLM multimodal review. Story-mode classification validates narrative structure matches actual scene composition. Evidence contracts on StoryBeat ensure claim-to-visual alignment. Composer emits structured diagnostics and rendered scene manifest consumed by Reviewer. Repair router maps quality failures to the correct existing agent for targeted fix. 10 new core modules, 0 new top-level agents. See `docs/adr/0023-job4-quality-gates-and-repair-routing.md` | P0 | MVP |
...
| PR-36 | Reviewer context and diagnostics enforcement: engine passes Composer `diagnostics`, `rendered_scene_manifest`, `story_beats`, and `word_timestamps` to Reviewer in both normal and repair rerun paths; manifest serialization prevents Pydantic dict-contract mismatch; timestamp-level semantic review enforces scene-to-beat alignment before LLM review. See `docs/adr/0024-reviewer-context-diagnostics-enforcement.md` | P0 | MVP |
| PR-37 | Pre-Visual-Director asset qualification boundary: a qualification layer (`clipper_agency/core/asset_qualification.py`, wired via the engine seam `_apply_asset_qualification` inside the orchestrator's `_run_visual_director_phase`) scores each beat's image candidates BEFORE the Visual Director consumes them, immutably rewrites each beat's candidate pool to the qualified set only so rejected candidates never reach VD's live per-beat surface, and writes a `qualification_report.json` artifact documenting verdicts (qualified/recovered/exhausted_text_card), recovery outcome, and reject reasons. When a beat has zero qualified candidates, a bounded RECOVER stage (`MAX_RECOVERY_CYCLES=1`) re-runs Segment Producer discovery for fresh candidates and re-scores them BEFORE falling back to a text card — Job #8 root cause was candidate REJECTION, not scarcity. Cache-key parity with VD's own scoring ensures VD's re-inspection of a pre-qualified candidate is a cache hit (0 double-VLM spend). Pure contract-enforcement orchestration: no new agent, no new gate, no schema change, no state-machine change. See `docs/adr/0026-v2.4.0-contract-enforcement-over-rebuild.md` and `docs/adr/0027-asset-qualification-inspection-delegation.md` | P0 | MVP |
| PR-38 | **Narrative-coverage contract gate (G7 GateNarrativeCoverage; FIX-8 cue-anchor):** assert Scriptwriter `narrative_structure` `word_range` fully covers `voiceover_text` (`[0, word_count-1]`, contiguous, in-bounds) BEFORE Voice Producer runs — job_18 root cause was a `word_range` covering only 24/76 words → 52 words / ~23 s of narration collapsed into one 25-second closing card. **FIX-8:** the LLM cannot reliably count its own words (job_19 over-index, job_20 under-index of the same gate), so the contract shifted — the Scriptwriter now emits `start_cue` (3-5 first words of each beat, verbatim from `voiceover_text`) and CODE derives `word_range` via the pure `clipper_agency/core/beat_anchor.py::derive_word_ranges` (fuzzy `max(Jaccard, LCS) ≥ 0.6`, deterministic; enclitics nya/kah/lah kept attached). Cue failures (`cue_not_found` / `cue_out_of_order`) route to Scriptwriter regen via the FIX-5 reason-based router. The FIX-1 in-place tail repair (<5 %) stays as defense-in-depth. In-place repair for tail <5 % of words; else hard-fail routing repair to **Scriptwriter** (not Visual Director). See `docs/adr/0030-inter-agent-contract-gates.md` | P0 | ADR 0030 |
| PR-39 | **Audio-as-master — audio never truncated:** drop the Composer's `-shortest` (which silently cut the last ~2.6 s of job_18's CTA) in favor of explicit `-t voiceover_duration`; pre-render-pad the visual track to `voiceover_duration_sec`; new G9.5 visual-coverage gate between VD and Composer; G10 `AUDIO_NOT_TRUNCATED` audio-stream re-probe. MoneyPrinterTurbo's audio-as-authoritative-timing-scalar + "use anyway, never freeze" loop-fill policy. See ADR 0030 | P0 | ADR 0030 |
| PR-40 | **Entity-binding rejection at the qualification/VD chokepoint:** shared `candidate_semantic_ranker` `WRONG_ENTITY` rule (subject_name must overlap beat `spoken_point` + `main_entities`), VLM `subject_name` required (missing ⇒ revise), `person_match` 0.8→0.6, and a `MAX_BEAT_DURATION_SEC`/max-scene cap forbidding a single 25 s card — kills the Jennifer-Coppen-for-Sarwendah class. CLIP image-text ranking deferred to Phase 27+. See ADR 0030 | P0 | ADR 0030 |
| PR-41 | **Reviewer per-scene detection:** extend `_check_av_sync` past the total-duration scalar with per-scene `ENTITY-VS-BIND` (manifest asset subject vs mapped beat), `AUDIO_NOT_TRUNCATED` audio re-probe, and `MAX-DWELL`/frozen-frame detection — so the Reviewer is no longer structurally blind to a frozen card / wrong face / truncated CTA. See ADR 0030. **Status (FIX-4):** ENTITY-VS-BIND + AUDIO_NOT_TRUNCATED reviewer gates COMPLETE (reuse FIX-2/FIX-3 machinery; `subject_name` threaded VD→manifest→Reviewer; `voiceover_duration_sec` wired on both engine reviewer paths); MAX-DWELL/frozen-frame DEFERRED to PR-44 (FIX-7 engagement) | P0 | ADR 0030 |
| PR-42 | **Timeline UNCOVERED_TAIL detection + MAX-beat cap:** `build_canonical_timeline` stops manufacturing 25 s mega-beats silently — detects uncovered gaps, caps any single beat at ~12 s, and surfaces a structured `UNCOVERED_TAIL` signal. Defense-in-depth behind PR-38. See ADR 0030 | P1 | ADR 0030 |
| PR-43 | **Repair-router root-cause routing:** route repair by failure REASON to the root agent (Scriptwriter for coverage, VD for entity/dwell, Composer for audio), force `narrative_structure` regen on coverage re-fail, bounded `MAX_REPAIR_CYCLES` + terminal fail (never "complete" garbage). See ADR 0030 | P0 | ADR 0030 |
| PR-44 | **Engagement gates — TikTok-post-worthy bar:** visual-change-density, hook-on-beat-0 (beat 0 is a real image not a title card; first change by 1.5 s), duration-band 21–42 s, monotony guard. Defends the five "AI low-effort tells". WARN+repair, not pipeline-death. See ADR 0030 | P1 | ADR 0030 |
