...
| PR-38 | **Narrative-coverage contract gate (G7 GateNarrativeCoverage):** assert Scriptwriter `narrative_structure` `word_range` fully covers `voiceover_text` (`[0, word_count-1]`, contiguous, in-bounds) BEFORE Voice Producer runs — job_18 root cause was a `word_range` covering only 24/76 words → 52 words / ~23 s of narration collapsed into one 25-second closing card. In-place repair for tail <5 % of words; else hard-fail routing repair to **Scriptwriter** (not Visual Director). See `docs/adr/0030-inter-agent-contract-gates.md` | P0 | ADR 0030 |
